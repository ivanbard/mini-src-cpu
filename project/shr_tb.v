...
module shr_tb;
    ...
    localparam ALU_ADD = 5'b00000, ALU_SHR = 5'b01000;
    ...
    initial begin
        $dumpfile("shr_tb.vcd"); $dumpvars(0, shr_tb);
        reset = 1; #25 reset = 0;
        wait(Present_state == Done); #40;
        $display("==============================================");
        $display("SHR Instruction Test: shr R7, R0, R4");
        $display("==============================================");
        $display("R0 (value)     = 0x%h", DUT.r0_out);
        $display("R4 (shift amt) = 0x%h (%0d)", DUT.r4_out, DUT.r4_out);
        $display("R7 (result)    = 0x%h (expected: 0x08000000)", DUT.r7_out);
        $display("Expected: 0x80000000 >> 4 = 0x08000000");
        if (DUT.r7_out == 32'h08000000) $display("TEST PASSED!");
