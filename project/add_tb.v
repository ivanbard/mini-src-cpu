...
module add_tb;
    ...
    always @(Present_state) begin
        case (Present_state)
            ...
            T1: begin mar_in <= 0; IncPC <= 0; Zin <= 0; bus_sel <= SEL_ZLOW; pc_in <= 1; Read <= 1; MDRin <= 1; Mdatain <= 32'h1A920000; end
            // T2: MDRout, IRin
            T2: begin pc_in <= 0; Read <= 0; MDRin <= 0; bus_sel <= SEL_MDR; ir_in <= 1; end
            // T3: R5out, Yin
            T3: begin ir_in <= 0; bus_sel <= SEL_R5; y_in <= 1; end
            // T4: R6out, ADD, Zin
            T4: begin y_in <= 0; bus_sel <= SEL_R6; alu_op <= ALU_ADD; Zin <= 1; end
            // T5: Zlowout, R2in
    ...
    initial begin
        $dumpfile("add_tb.vcd"); $dumpvars(0, add_tb);
        reset = 1; #25 reset = 0;
