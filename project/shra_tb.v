...
module shra_tb;
    ...
    localparam ALU_ADD = 5'b00000, ALU_SHRA = 5'b01001;
    ...
    always @(Present_state) begin
        case (Present_state)
            ...
            // Load R0 with 0xF0000000 (negative value to shift)
            Reg_load1a: begin Mdatain <= 32'hF0000000; Read <= 1; MDRin <= 1; end
            ...
            T1: begin mar_in <= 0; IncPC <= 0; Zin <= 0; bus_sel <= SEL_ZLOW; pc_in <= 1; Read <= 1; MDRin <= 1; Mdatain <= 32'hAB808000; end
            // T2: MDRout, IRin
            T2: begin pc_in <= 0; Read <= 0; MDRin <= 0; bus_sel <= SEL_MDR; ir_in <= 1; end
            // T3: R0out, Yin
            T3: begin ir_in <= 0; bus_sel <= SEL_R0; y_in <= 1; end
            // T4: R4out, SHRA, Zin
            T4: begin y_in <= 0; bus_sel <= SEL_R4; alu_op <= ALU_SHRA; Zin <= 1; end
            // T5: Zlowout, R7in
    ...
    initial begin
        $dumpfile("shra_tb.vcd"); $dumpvars(0, shra_tb);
        reset = 1; #25 reset = 0;
        wait(Present_state == Done); #40;
        $display("==============================================");
        $display("SHRA Instruction Test: shra R7, R0, R4");
        $display("==============================================");
        $display("R0 (value)     = 0x%h (negative)", DUT.r0_out);
        $display("R4 (shift amt) = 0x%h (%0d)", DUT.r4_out, DUT.r4_out);
        $display("R7 (result)    = 0x%h (expected: 0xFF000000)", DUT.r7_out);
        $display("Expected: 0xF0000000 >>> 4 = 0xFF000000 (sign extended)");
        if (DUT.r7_out == 32'hFF000000) $display("TEST PASSED!");
