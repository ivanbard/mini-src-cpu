// mfhi R5  |  HI=0xDEADBEEF  |  expect R5=0xDEADBEEF
`timescale 1ns/10ps
...
module mfhi_tb;
    ...
    parameter Default=5'd0, HI_ld=5'd1,
    ...
    always @(posedge clk) case(Present_state)
        Default: Present_state <= HI_ld;
        HI_ld:   Present_state <= T0;
        T0:      Present_state <= T1;
        T1:      Present_state <= T2;
        T2:      Present_state <= T3;
    ...
    always @(Present_state) begin
        clear_sigs;
        case(Present_state)
            HI_ld: begin c_val<=32'hDEADBEEF; bus_sel<=SEL_C; hi_in<=1; end
            T0: begin bus_sel<=SEL_PC; mar_in<=1; IncPC<=1; end
            T1: begin Read<=1; MDRin<=1; ram_in<=1; end
            T2: begin bus_sel<=SEL_MDR; ir_in<=1; end
            T3: begin bus_sel<=SEL_HI; Gra<=1; Rin<=1; end
        endcase
    end
    
    initial begin
        $dumpfile("mfhi_tb.vcd"); $dumpvars(0,mfhi_tb);
        DUT.ram_inst.mem[9'h000]=32'h5A800000;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("MFHI: mfhi R5  [HI=0xDEADBEEF]");
        $display("  HI  = 0x%08h  (exp 0xDEADBEEF)", DUT.key_regs_inst.HI.d_out);
        $display("  R5  = 0x%08h  (exp 0xDEADBEEF)", DUT.rf.r5_out);
        if(DUT.rf.r5_out===32'hDEADBEEF) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
