// jr R12  |  preload R12=0xFF, PC=0x10  |  expect PC=0xFF
`timescale 1ns/10ps
...
module jr_tb;
    ...
    parameter Default=5'd0, RL0=5'd1, RL1=5'd2, RL2=5'd3, RL3=5'd4, RL4=5'd5,
              RL5=5'd6, T0=5'd7, T1=5'd8, T2=5'd9, T3=5'd10, Done=5'd11;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            RL3: ; // idle — IR.d_out settles to dummy (0x06000000)
            RL4: begin c_val<=32'h000000FF; bus_sel<=SEL_C; Gra<=1; Rin<=1; end
            ...
            T3: begin Gra<=1; Rout<=1; pc_in<=1; end
        endcase
    end
    
    initial begin
        $dumpfile("jr_tb.vcd"); $dumpvars(0,jr_tb);
        DUT.ram_inst.mem[9'h1FF]=32'h06000000;
        DUT.ram_inst.mem[9'h010]=32'h4E000000;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("JR: jr R12  [R12=0xFF, PC preload=0x10]");
        $display("  R12 = 0x%08h  (exp 0x000000FF)", DUT.rf.r12_out);
        $display("  MAR = 0x%08h  (exp 0x00000010 during fetch)", mar_val);
        $display("  PC  = 0x%08h  (exp 0x000000FF)", pc_val);
        if(pc_val===32'h000000FF) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
