// in R5  |  device_in=0x12345678  |  expect R5=0x12345678
`timescale 1ns/10ps
...
module in_tb;
    ...
    parameter Default=5'd0, T0=5'd1, T1=5'd2, T2=5'd3,
              T3=5'd4, T4=5'd5, Done=5'd6;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            T3: begin inport_in<=1; end
            T4: begin bus_sel<=SEL_INPORT; Gra<=1; Rin<=1; end
        endcase
    end
    
    initial begin
        $dumpfile("in_tb.vcd"); $dumpvars(0,in_tb);
        device_in=32'h12345678;
        DUT.ram_inst.mem[9'h000]=32'h6A800000;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("IN: in R5  [device_in=0x12345678]");
        $display("  InPort = 0x%08h  (exp 0x12345678)", DUT.inport_inst.inport_val);
        $display("  R5     = 0x%08h  (exp 0x12345678)", DUT.rf.r5_out);
        if(DUT.rf.r5_out===32'h12345678) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
