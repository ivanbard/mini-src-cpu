// st 0x1F(R6), R6  |  R6=0x63, EA=0x82  |  mem[0x82] should become 0x63
`timescale 1ns/10ps
...
module st_case2_tb;
    ...
    parameter Default=5'd0, RL0=5'd1, RL1=5'd2, RL2=5'd3, RL3=5'd4, RL4=5'd5,
              ...
              T5=5'd11, T6=5'd12, T7=5'd13, Done=5'd14;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            RL3: ; // idle — IR.d_out settles to dummy (0x03000000)
            RL4: begin c_val<=32'h00000063; bus_sel<=SEL_C; Gra<=1; Rin<=1; end
            T0: begin bus_sel<=SEL_PC; mar_in<=1; IncPC<=1; end
            T1: begin Read<=1; MDRin<=1; ram_in<=1; end
            T2: begin bus_sel<=SEL_MDR; ir_in<=1; end
            T3: begin Grb<=1; BAout<=1; Rout<=1; y_in<=1; end
            T4: begin Cout<=1; bus_sel<=SEL_C; alu_op<=OP_ADD; Zin<=1; end
            T5: begin bus_sel<=SEL_ZLOW; mar_in<=1; end
            T6: begin Gra<=1; Rout<=1; MDRin<=1; end
            T7: begin bus_sel<=SEL_MDR; ram_in<=1; end
        endcase
    end
    
    initial begin
        $dumpfile("st_case2_tb.vcd"); $dumpvars(0,st_case2_tb);
        DUT.ram_inst.mem[9'h1FF]=32'h03000000;
        DUT.ram_inst.mem[9'h000]=32'h1330001F;
        DUT.ram_inst.mem[9'h082]=32'h000000A7;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("ST Case 2: st 0x1F(R6), R6  [R6=0x63, mem[0x82] was 0xA7]");
        $display("  R6       = 0x%08h  (exp 0x00000063)", DUT.rf.r6_out);
        $display("  MAR      = 0x%08h  (exp 0x00000082)", mar_val);
        $display("  mem[0x82]= 0x%08h  (exp 0x00000063)", DUT.ram_inst.mem[9'h082]);
        if(DUT.ram_inst.mem[9'h082]===32'h00000063) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
