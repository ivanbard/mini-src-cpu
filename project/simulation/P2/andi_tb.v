// andi R7, R4, 0x71  |  R4=0xF0  |  expect R7=0x70
`timescale 1ns/10ps
...
module andi_tb;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            T4: begin Cout<=1; bus_sel<=SEL_C; alu_op<=OP_AND; Zin<=1; end
            T5: begin bus_sel<=SEL_ZLOW; Gra<=1; Rin<=1; end
        endcase
    end

    initial begin
        $dumpfile("andi_tb.vcd"); $dumpvars(0,andi_tb);
        DUT.ram_inst.mem[9'h1FF] = 32'h02000000;
        DUT.ram_inst.mem[9'h000] = 32'h33A00071;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("ANDI: andi R7, R4, 0x71");
        $display("  R4  = 0x%08h  (exp 0x000000F0)", DUT.rf.r4_out);
        $display("  C_ext= 0x%08h  (exp 0x00000071)", DUT.extend_c_inst.C_extended);
        $display("  R7  = 0x%08h  (exp 0x00000070)", DUT.rf.r7_out);
        if(DUT.rf.r7_out===32'h00000070) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
