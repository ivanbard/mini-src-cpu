// out R7  |  R7=0xBEEFCAFE  |  expect OutPort=0xBEEFCAFE
`timescale 1ns/10ps
...
module out_tb;
    ...
    parameter Default=5'd0, RL0=5'd1, RL1=5'd2, RL2=5'd3, RL3=5'd4, RL4=5'd5,
              T0=5'd6, T1=5'd7, T2=5'd8, T3=5'd9, Done=5'd10;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            RL3: ; // idle — IR.d_out settles to dummy (0x03800000)
            RL4: begin c_val<=32'hBEEFCAFE; bus_sel<=SEL_C; Gra<=1; Rin<=1; end
            T0: begin bus_sel<=SEL_PC; mar_in<=1; IncPC<=1; end
            T1: begin Read<=1; MDRin<=1; ram_in<=1; end
            T2: begin bus_sel<=SEL_MDR; ir_in<=1; end
            T3: begin Gra<=1; Rout<=1; outport_in<=1; end
        endcase
    end
    
    initial begin
        $dumpfile("out_tb.vcd"); $dumpvars(0,out_tb);
        DUT.ram_inst.mem[9'h1FF]=32'h03800000;
        DUT.ram_inst.mem[9'h000]=32'h73800000;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("OUT: out R7  [R7=0xBEEFCAFE]");
        $display("  R7      = 0x%08h  (exp 0xBEEFCAFE)", DUT.rf.r7_out);
        $display("  OutPort = 0x%08h  (exp 0xBEEFCAFE)", outport_val);
        if(outport_val===32'hBEEFCAFE) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
