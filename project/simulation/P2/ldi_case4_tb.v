// ldi R0, 0x72(R2)  |  R2=0x57  |  expect R0=0xC9
`timescale 1ns/10ps
...
module ldi_case4_tb;
    ...
    initial begin
        $dumpfile("ldi_case4_tb.vcd"); $dumpvars(0,ldi_case4_tb);
        DUT.ram_inst.mem[9'h1FF]=32'h01000000;
        DUT.ram_inst.mem[9'h000]=32'h08100072;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("LDI Case 4: ldi R0, 0x72(R2)  [R2=0x57]");
        $display("  R2  = 0x%08h  (exp 0x00000057)", DUT.rf.r2_out);
        $display("  R0  = 0x%08h  (exp 0x000000C9)", DUT.rf.r0_out);
        if(DUT.rf.r0_out===32'h000000C9) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
