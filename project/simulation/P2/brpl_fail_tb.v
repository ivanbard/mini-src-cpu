// brpl R3, 48 (not-taken demo) — branch if R3 >= 0
`timescale 1ns/10ps
...
module brpl_fail_tb;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            RL3: ;
            RL4: begin c_val<=r3_val; bus_sel<=SEL_C; Gra<=1; Rin<=1; end
    ...
    initial begin
        $dumpfile("brpl_fail_tb.vcd"); $dumpvars(0,brpl_fail_tb);
        DUT.ram_inst.mem[9'h1FF]=32'h01800000; // dummy Ra=R3
        DUT.ram_inst.mem[9'h000]=32'h41900030;
        r3_val = 32'hFFFFFFFB;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("BRPL FAIL TB (not-taken demo): brpl R3, 48");
        $display("  [IR[20:19]=10]");
        $display("  R3  = 0x%08h  (exp 0xFFFFFFFB)", DUT.rf.r3_out);
        $display("  CON = %b  (exp 0)", con);
        $display("  PC  = 0x%08h  (exp 0x00000001)", pc_val);
        if(con===1'b0 && pc_val===32'h00000001) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
