// ld R0, 0x72(R2)  |  R2=0x57, mem[0xC9]=0x2B  |  expect R0=0x2B
`timescale 1ns/10ps
...
module ld_case2_tb;
    ...
    always @(Present_state) begin
        ...
        case(Present_state)
            ...
            RL3: ; // idle — IR.d_out settles to dummy (0x01000000)
            RL4: begin c_val<=32'h00000057; bus_sel<=SEL_C; Gra<=1; Rin<=1; end
            ...
            T6: begin Read<=1; MDRin<=1; ram_in<=1; end
            T7: begin bus_sel<=SEL_MDR; Gra<=1; Rin<=1; end
        endcase
    end
    
    initial begin
        $dumpfile("ld_case2_tb.vcd"); $dumpvars(0,ld_case2_tb);
        DUT.ram_inst.mem[9'h1FF]=32'h01000000;
        DUT.ram_inst.mem[9'h000]=32'h00100072;
        DUT.ram_inst.mem[9'hC9] =32'h0000002B;
        reset=1; #25; reset=0;
        wait(Present_state==Done); #40;
        $display("============================================");
        $display("LD Case 2: ld R0, 0x72(R2)  [R2=0x57, mem[0xC9]=0x2B]");
        $display("  R2  = 0x%08h  (exp 0x00000057)", DUT.rf.r2_out);
        $display("  MAR = 0x%08h  (exp 0x000000C9)", mar_val);
        $display("  R0  = 0x%08h  (exp 0x0000002B)", DUT.rf.r0_out);
        if(DUT.rf.r0_out===32'h0000002B) $display("  >>> PASSED <<<");
        else $display("  >>> FAILED <<<");
        $display("============================================");
        #20; $finish;
    end
