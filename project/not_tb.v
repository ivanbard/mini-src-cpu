...
module not_tb;
    ...
    localparam ALU_ADD = 5'b00000, ALU_NOT = 5'b00110;

    // NOT uses T0-T4 (5 steps, unary operation)
    ...
    always @(Present_state) begin
        case (Present_state)
            ...
            // Load R7 with 0x0000FFFF (value to bitwise NOT)
            ...
            T1: begin mar_in <= 0; IncPC <= 0; Zin <= 0; bus_sel <= SEL_ZLOW; pc_in <= 1; Read <= 1; MDRin <= 1; Mdatain <= 32'hE5C00000; end
            // T2: MDRout, IRin
            T2: begin pc_in <= 0; Read <= 0; MDRin <= 0; bus_sel <= SEL_MDR; ir_in <= 1; end
            // T3: R7out, NOT, Zin (unary op - only one operand)
            T3: begin ir_in <= 0; bus_sel <= SEL_R7; alu_op <= ALU_NOT; Zin <= 1; end
            // T4: Zlowout, R4in
    ...
    initial begin
        $dumpfile("not_tb.vcd"); $dumpvars(0, not_tb);
        reset = 1; #25 reset = 0;
        wait(Present_state == Done); #40;
        $display("==============================================");
        $display("NOT Instruction Test: not R4, R7");
        $display("==============================================");
        $display("R7 (source)     = 0x%h", DUT.r7_out);
        $display("R4 (result)     = 0x%h (expected: 0xFFFF0000)", DUT.r4_out);
        $display("Expected: ~0x0000FFFF = 0xFFFF0000");
        if (DUT.r4_out == 32'hFFFF0000) $display("TEST PASSED!");
