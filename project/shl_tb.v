...
module shl_tb;
    ...
    localparam ALU_ADD = 5'b00000, ALU_SHL = 5'b01010;
    ...
    always @(Present_state) begin
        case (Present_state)
            ...
            // Load R0 with 0x00000001 (value to shift)
            Reg_load1a: begin Mdatain <= 32'h00000001; Read <= 1; MDRin <= 1; end
            Reg_load1b: begin Read <= 0; MDRin <= 0; bus_sel <= SEL_MDR; rin <= 16'b0000000000000001; end  // R0in
            // Load R4 with 8 (shift amount)
            Reg_load2a: begin rin <= 16'b0; bus_sel <= 5'b0; Mdatain <= 32'h00000008; Read <= 1; MDRin <= 1; end
            ...
            T1: begin mar_in <= 0; IncPC <= 0; Zin <= 0; bus_sel <= SEL_ZLOW; pc_in <= 1; Read <= 1; MDRin <= 1; Mdatain <= 32'hB3808000; end
            // T2: MDRout, IRin
            T2: begin pc_in <= 0; Read <= 0; MDRin <= 0; bus_sel <= SEL_MDR; ir_in <= 1; end
            // T3: R0out, Yin
            T3: begin ir_in <= 0; bus_sel <= SEL_R0; y_in <= 1; end
            // T4: R4out, SHL, Zin
            T4: begin y_in <= 0; bus_sel <= SEL_R4; alu_op <= ALU_SHL; Zin <= 1; end
            // T5: Zlowout, R7in
    ...
    initial begin
        $dumpfile("shl_tb.vcd"); $dumpvars(0, shl_tb);
        reset = 1; #25 reset = 0;
        wait(Present_state == Done); #40;
        $display("==============================================");
        $display("SHL Instruction Test: shl R7, R0, R4");
        $display("==============================================");
        $display("R0 (value)     = 0x%h", DUT.r0_out);
        $display("R4 (shift amt) = 0x%h (%0d)", DUT.r4_out, DUT.r4_out);
        $display("R7 (result)    = 0x%h (expected: 0x00000100)", DUT.r7_out);
        $display("Expected: 0x00000001 << 8 = 0x00000100");
        if (DUT.r7_out == 32'h00000100) $display("TEST PASSED!");
