...
module ror_tb;
    ...
    localparam ALU_ADD = 5'b00000, ALU_ROR = 5'b01011;
    ...
    always @(Present_state) begin
        case (Present_state)
            ...
            // Load R0 with 0x00000001 (value to rotate)
            ...
            T1: begin mar_in <= 0; IncPC <= 0; Zin <= 0; bus_sel <= SEL_ZLOW; pc_in <= 1; Read <= 1; MDRin <= 1; Mdatain <= 32'hBB808000; end
            // T2: MDRout, IRin
            T2: begin pc_in <= 0; Read <= 0; MDRin <= 0; bus_sel <= SEL_MDR; ir_in <= 1; end
            // T3: R0out, Yin
            T3: begin ir_in <= 0; bus_sel <= SEL_R0; y_in <= 1; end
            // T4: R4out, ROR, Zin
            T4: begin y_in <= 0; bus_sel <= SEL_R4; alu_op <= ALU_ROR; Zin <= 1; end
            // T5: Zlowout, R7in
    ...
    initial begin
        $dumpfile("ror_tb.vcd"); $dumpvars(0, ror_tb);
        reset = 1; #25 reset = 0;
        wait(Present_state == Done); #40;
        $display("==============================================");
        $display("ROR Instruction Test: ror R7, R0, R4");
        $display("==============================================");
        $display("R0 (value)      = 0x%h", DUT.r0_out);
        $display("R4 (rotate amt) = 0x%h (%0d)", DUT.r4_out, DUT.r4_out);
        $display("R7 (result)     = 0x%h (expected: 0x10000000)", DUT.r7_out);
        $display("Expected: 0x00000001 ROR 4 = 0x10000000");
        if (DUT.r7_out == 32'h10000000) $display("TEST PASSED!");
